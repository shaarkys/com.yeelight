  ...
  discover() {
    try {
      ...
      return new Promise(resolve => {
        ...
        var broadcast = () => advertisements.send(message, 0, message.length, 1982, "239.255.255.250", (err) => {
          if (err) {
              ...
              if (result !== 'no devices') {
                ...
                Object.keys(added_devices).forEach((key) => {
                  ...
                  if (result.message_type !== 'discover' && addedDevice.getData().id === result.device.id) {
                    if (addedDevice.getSetting('address') !== result.device.address || addedDevice.getSetting('port') !== result.device.port) {
                      addedDevice.setSettings({ address: result.device.address, port: result.device.port });
                      this.homey.app.log(`[Util] Updated device address: ${result.device.id} to ${result.device.address}:${result.device.port}`);
                    }
