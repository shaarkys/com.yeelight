...
class YeelightApp extends Homey.App {
  onInit() {
    this.homey.app.log('Initializing Yeelight app ...');
    ...
    this.updateEventsInterval = setInterval(async () => {
    ...
    this.homey.flow.getActionCard('yeelightDefault').registerRunListener(async (args) => {
      return await args.device.sendCommand(args.device.getData().id, '{"id":1,"method":"set_default","params":[]}');
    });

    this.homey.flow.getActionCard('yeelightFlowBrightness').registerRunListener(async (args) => {
    ...
    this.homey.flow.getActionCard('yeelightTemperatureScene').registerRunListener(async (args) => {
    ...
    this.homey.flow.getActionCard('yeelightColorScene').registerRunListener(async (args) => {
    ...
    this.homey.flow.getActionCard('yeelightCustomCommand').registerRunListener(async (args) => {
      return await args.device.sendCommand(args.device.getData().id, args.command);
    });

    this.homey.flow.getActionCard('yeelightNightMode').registerRunListener(async (args) => {
      const night_mode = args.mode == 'night';
      return await args.device.triggerCapabilityListener('night_mode', night_mode);
    });

    this.homey.flow.getActionCard('yeelightAmbilightSwitch').registerRunListener(async (args) => {
      if (args.switch == 'on') {
        var switch_bg = true;
      } else {
        var switch_bg = false;
    ...
    this.homey.flow.getActionCard('yeelightAmbilightDim').registerRunListener(async (args, state, opts) => {
    ...
    this.homey.flow.getActionCard('yeelightAmbilightTemperature').registerRunListener(async (args) => {
      return await args.device.triggerCapabilityListener('light_temperature.bg', args.temperature);
    });

    this.homey.flow.getActionCard('yeelightSaveState').registerRunListener(async (args) => {
      return await args.device.saveState(args.device);
    });

    this.homey.flow.getActionCard('yeelightSetState').registerRunListener(async (args) => {
      return await args.device.setState(args.device);
    });

    this.homey.app.log('Yeelight app initialized.');
  }
}
